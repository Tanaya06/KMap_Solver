...
int IsPowerOfTwo(int n); int IsDontCare(int MT);
void ShowResult(); void Recursion_For_Loop(int m); 
...
int main() 
{
	...
	while(NumberOfDontCare>=NumberOfAllMinterm || NumberOfDontCare<0)
	{
 	...
 	MintermIndicesDecimal=(int *)malloc(NumberOfAllMinterm*sizeof(int)); /* Record the decimal indices representing each minterm */
	MintermIndicesDecimal_DontCare=(int *)malloc(NumberOfDontCare*sizeof(int));  /* Record the decimal indices representing Don't-Care minterms */
	...
	{ 
		...
		Minterm_Binary[i][NumberOfVariable+1]=0;       /* '0' means it hasn't been grouped, '1' means it has been grouped with other terms */   
		Minterm_Binary[i][NumberOfVariable+2]=MintermIndicesDecimal[i];  /* this is its original minterm */   
		Minterm_Binary[i][NumberOfVariable+3]=MintermIndicesDecimal[i];  /* this is all the minterms it consists of */  
	} 
 	/***********Prepare the first column for grouping***********/  
	Column=(int ****)malloc((NumberOfVariable+1)*sizeof(int***));  
	for(i=0;i<NumberOfVariable+1;i++)  
	{   
		Column[i]=(int ***)malloc((NumberOfVariable+1-i)*sizeof(int**));     /* Column[i] contains all the terms in the (i+1)th column */  
	}   
	for(i=0;i<NumberOfVariable+1;i++)  
	{   
		for(j=0;j<NumberOfVariable+1-i;j++)   
		{    
			Column[i][j]=(int **)malloc(Combination(NumberOfVariable,i,j)*sizeof(int*)); /* Column[i][j] contains all the terms with j '1's in their binary indices in the (i+1)th column */ 
   			for(k=0;k<Combination(NumberOfVariable,i,j);k++)    
			{     
				Column[i][j][k]=NULL;             
				/* Column[i][j][k] represents a term with in the j '1's in their binary indices in the (i+1)th column */    
			}
		}  
	}  
	for(i=0;i<NumberOfVariable+1;i++)
	{   
		...
		{    
			...
			{     
				Column[0][i][k++]=Minterm_Binary[j];      /* Prepare the first grouping column */    
			}     
		}
	} 
			...
			{      
				...
				{        
					...
					{         
						...
						{          
							LogicProbe=0-i;  /*This LogicProbe is used to check whether this two terms has the same positions of '-'(which is represented by '2')*/          
							for(n=1;n<=i;n++)           
							for(p=1;p<=i;p++)            
	...
	/***********In the PI Chart, find the minterms which only occurs once, and select the PIs which contain these minterms as EPIs and record them. Then set NumberCounter of this minterms to 0***********/  
	for(i=0;i<pow(2,NumberOfVariable);i++)   
	...
	NumberOfRemainingMT=0;  for(i=0;i<pow(2,NumberOfVariable);i++)   
	...
	for(i=0,j=0;j<pow(2,NumberOfVariable);j++)   if(NumberCounter[j]!=0)   
	{    
	...
	/***********ReducedPIChart[i][j] represent the information of Reduced PI Chart('1' means picked, '0' means unpicked)***********/  
	...
	{   
		...
		/* For[i] will be used in the function 'Recursion_For_Loop(int m)' */   
		for(i=0;i<NumberOfRemainingMT;i++)   
		{    
			For[i]=-1;   
		}
		...
		NoOfPIForEPI=(int *)malloc(NumberOfPossibleEPI*sizeof(int)); /* NoOfPIForEPI[i] will count how many PIs are in each combination that covers all minterms */   
		for(i=0;i<NumberOfPossibleEPI;i++)    
		NoOfPIForEPI[i]=0;
		for(i=0;i<NumberOfPossibleEPI;i++)    
		...
		/***********Find the combination which require the least number of PIs to cover all minterms***********/    
